...
//---------------------------------------NOTE:---------------------------------------
// Modified by Shania Stewart (July 19, 2017).
// This program is a modified version of the Arduino_LoRa_Simple_temp sketch. 
// The modifications are used to achieve consistent run times for this program on 
// the order of microseconds.
// Please note that this does not currently work with the Low Power options. 
// I will likely make another modification sketch for use with Low Power options.
// However, this has been used for my own personal testing primarily. If you wish 
// to edit, feel free, but be aware that adding/deleting code will change the run 
// times - this includes things like print statements. Comments do not contribute 
// to the time or memory of the program.
// Enjoy!
//-----------------------------------------------------------------------------------
...
//************************************************************************************
//idle time in microseconds with ten second delay:
unsigned long waitTime = ((idlePeriodInMin*60)+10)*1000000; 
unsigned long initTime = 0L; //time at which program starts the next loop
unsigned long prevInitTime = 0L; //used for runtime reading
//unsigned long timeBetween = 0L; //obselete
unsigned long endTime = 0L; //used for runtime reading - when program completely ends.
unsigned long transmitTime = 0L; //time when packet was sent
int rollover_flag = 0;
//************************************************************************************
char float_str[20];
uint8_t message[100];
...
#endif
  //************************************************************************************
  //For testing when the program starts for the first time.
  
  //initTime = micros();
  //PRINT_CSTSTR("%s"," Actual Start Time \n");
  //PRINT_VALUE("%ld", initTime);
  //PRINTLN;
  //PRINT_CSTSTR("%s","Idle period in milliseconds\n");
  //PRINT_VALUE("%ld",(unsigned long)idlePeriodInMin*60*1000);
  //PRINTLN;
  //************************************************************************************
...
void loop(void)
{
  ...
  //************************************************************************************
 //Rollover protection. Sometimes nexTransmissionTime will overflow with the added 
 //waitTime, so for the condition that starts the loop, this would cause consitent 
 //transmissions until the micros() rollover. This block should stop this problem.
 while (rollover_flag == 1){
    if (micros() <= nextTransmissionTime){
      rollover_flag = 0;
      break;
    }
 }
...
#endif
	//************************************************************************************
	// Initially used to see times before wait points were added. Somewhat obselete.
	
	//if (transmitTime != 0){ //Use initTime above for first run.
		
      initTime = micros();
	  
      PRINT_CSTSTR("%s","***\n");
	  
      //timeBetween = initTime - transmitTime;
	  
      PRINT_CSTSTR("%s","Current Start Time\n");
      PRINT_VALUE("%ld", initTime);
      PRINTLN;
	  
      //PRINT_CSTSTR("%s","Time between last transmission\n");
      //PRINT_VALUE("%ld", timeBetween);
      //PRINTLN;
	  
      //PRINT_CSTSTR("%s","Expected vs Actual diff for start\n");
      //PRINT_VALUE("%ld", initTime - nextTransmissionTime);
      //PRINTLN;
	//}
	//************************************************************************************
      ...
      //&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
	  //first wait point
      while ((micros()-initTime) <= 1011000){

      }
      
      PRINT_VALUE("%ld", (unsigned long)micros()-initTime);
      PRINTLN; 
      ...
      endSend=micros();
      //&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
	  //prints when packet was sent
      PRINT_VALUE("%ld", transmitTime-initTime);
      PRINTLN;

	  //third wait point
      while ((micros()-initTime) <= 4173000){ 
	  ...
	  //finds runtime of last program loop
	  
      PRINT_CSTSTR("%s","Full Runtime of Previous Program\n");
      PRINT_VALUE("%ld", endTime - prevInitTime);
      PRINTLN; 
      prevInitTime = initTime;
      
      PRINT_CSTSTR("%s","Actual Idle Time\n");
      PRINT_VALUE("%ld", initTime - endTime); //you will have to ignore first time
      PRINTLN;//not sure how this will behave with long idle times 
      // (although to my understanding it may not matter)
	  ...
	  //****************************************************************************
	  // Removed random interval for consistency. 
    //adding the next if statement for rollover protection:
    nextTransmissionTime=micros()+waitTime; //changed to micros and no random added time
    if (nextTransmissionTime <= waitTime){
      rollover_flag = 1; //flag set to indicate the need of the rollover protection block.
    }
    else{
      rollover_flag = 0; //redundant, but could affect timing
    }
      PRINT_VALUE("%ld", nextTransmissionTime);
      PRINTLN;
      //Evidence of some collisions, so may increase beginning and ending wait times slightly.
	    //Possible solution: add fixed value to waitTime (say, 50 milliseconds?).
      endTime = micros();
      //****************************************************************************
  }
#endif
